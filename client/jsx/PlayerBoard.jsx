    ...
    render() {
        if (this.props.player) {
            ...
            return (<div className="player-board">
                <div>
                    <p>Total: ${player.notes}</p>
                    <div>
                        {(gameService.currentPlayer === player.id) &&
                        <button onClick={() => {window.scrollTo(0,0);this.setState({sendMoneyDialog: true})}}>Send money to player /
                            bank</button>}
                        {(player.id === 1) &&
                        <button onClick={() => {window.scrollTo(0,0);this.setState({sendMoneyDialog: true})}}>Take money from
                            bank</button>}
                    </div>
                </div>
